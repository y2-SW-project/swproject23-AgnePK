        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex">
                ...
                <form action="{{ route('jewellery.destroy', $jewellery) }}" method="post">
                    @method('delete')
                    @csrf
                    <button type="submit" class="btn btn-danger ml-4" onclick="return confirm('Are you sure you wish to delete this jewellery?')">Delete jewellery</button>
            </div>
            ...
            <div class="my-6 p-6 bg-white border-b border-gray-200 shadow-sm sm:rounded-lg flex">
                <div class="">
                    ...
                    <img class="border-b border-gray-400 shadow-lg sm:rounded-lg mt-5" src="{{ asset('storage/images/' . $jewellery->img) }}" width="350">
                </div>

                <div class="ml-6">
                    <p class="mt-3 whitespace-">{{$jewellery->name}}</p>
                    <p class="mt-3">Find us at: {{$jewellery->category}}</p>
                    <p class="mt-3 font-serif text-2xl">€{{$jewellery->material }}</p>
                </div>
